...
def main(args=None):
    ...
    subparser_ec_io.add_argument("-o", "--output_fn", required=True, help="Path the output eventalign collapsed tsv file")
    ...
    subparser_fm_io.add_argument("-o", "--output_fn", required=True, help="Path the output reference position tsv file")
    ...
    subparser_fm_rp = subparser_fm.add_argument_group("Run parameters options")
    subparser_fm_rp.add_argument("-c", "--call_threshold", type=float, default=2.5, help="Log likelihood ratio threshold (default: %(default)s)")
    subparser_fm_rp.add_argument("-s", "--split_groups", default=False, action='store_true', help = "split multi-cpg groups (default: %(default)s)")
    ...
    for sp in [subparser_ec, subparser_fm]:
        ...
        sp_other = sp.add_argument_group("Other options")
        sp_other.add_argument("-t", "--threads", default=4, type=int, help="Total number of threads. 1 thread is used for the reader and 1 for the writer (default: %(default)s)")
